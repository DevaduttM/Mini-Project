    ...
    <div className="flex flex-col items-center justify-center h-screen bg-[#F7F3F0] p-6">
      <div className="bg-white shadow-md rounded-xl h-[90%] p-6 w-full max-w-4xl border border-gray-300 relative">
        ...
        <div className="grid grid-cols-1 h-3/4 md:grid-cols-2 gap-6">
          ...
          <div onClick={() => fileInputRef.current.click()} className="border-2 border-dashed cursor-pointer border-gray-400 rounded-xl flex flex-col items-center justify-center p-6 bg-[#FDFBFA] shadow-sm relative">
            <p className="text-lg text-gray-700 mb-4 font-serif">Upload Video</p>
            ...
            <input type="file" ref = {fileInputRef} onChange={handleFileChange} className='w-full hidden' />
            </div>
            <p className='text-black'>{file? file.name : "No file selected"}</p>
            <button className='w-[30%] h-10 text-black bg-[#dfb8a6] mt-5 rounded-xl' onClick={handleFileUpload}>Upload</button>
          </div>
        </div>
      </div>
    </div>
